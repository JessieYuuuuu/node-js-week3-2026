...
// ───────────────────────────────────────────────────────────
const levelOptions = ["VIP", "normal"]; // 可使用的 level 選項
...
function validateBody(body) {
  ...
  if (!level) errors.push("缺 level");
  else if (!levelOptions.includes(level)) errors.push("錯誤的 level");
...
router.get("/", (req, res) => {
  // 未篩選回傳全部
  if (!req.query.level) return res.status(200).json(members);
  // 非可使用選項回 400
  if (!["VIP", "normal"].includes(req.query.level))
    return res.status(400).json({ error: "無效的 level" });
  // 篩選
...
router.put("/:id", (req, res) => {
  ...
  if (idx === -1) return res.status(404).json({ error: "會員不存在" });
  const updatedMember = { ...members[idx], ...req.body };
